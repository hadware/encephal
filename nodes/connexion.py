...
class ConvolutionalConnexion(PipeNode):
    ...
    def __init__(self, input_datasink, kernel_shape, zero_padding = ZeroPadding.VALID):
    ...
    '''Notice: FFT is generally much faster than convolve for large arrays (n > ~500), but can be slower when only a few output values are needed
    ...
    def learn(self, alpha):
        ...
        if self.zero_padding == ZeroPadding.VALID:
            self.kernel[:] += fftconvolve(self.input_socket.prop_data, self.output_socket.backprop_data, mode=self.zero_padding.value[1])
        elif self.zero_padding == ZeroPadding.FULL:
            self.kernel[:] += fftconvolve(self.output_socket.backprop_data, self.input_socket.prop_data,mode=self.zero_padding.value[2])
...
class PoolingConnexion(PipeNode):
    ...
    pooling_shape : the shape of the kernel thanks to we will pool on the input data
    stride_shape : the shape of the offset we use between each pooling
    """
    def __init__(self, input_datasink, pooling_shape, stride_shape, pooling_function = MaxPooling):
    ...
    def propagation(self, learning):
        self.output_socket.prop_data[:] += self.pooling_function.pool(self.input_socket.prop_data,self.output_shape,self.pooling_shape,self.stride_shape)
